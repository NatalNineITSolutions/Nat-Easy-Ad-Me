...
class MediaHelper
{
    ...
    public static function insert_media_image($request, $type = 'admin', $file_field_name = 'file')
    {
        if ($request->hasFile($file_field_name)) {
            ...
            // Retrieve the file from the request using Laravel's file() method
            $image = $request->file($file_field_name);

            // Validate that the uploaded file is valid
            if (!$image->isValid()) {
                return back()->withErrors(['image' => 'Uploaded file is not valid.']);
            }

            // Instead of getRealPath(), use getPathname() to get the absolute path
            $imagePath = $image->getPathname();
            if (!$imagePath || !file_exists($imagePath)) {
                return back()->withErrors(['image' => 'File path is empty or file does not exist.']);
            }

            // Get image dimensions and file size using the image path
            $image_dimension = getimagesize($imagePath);
            $image_width     = $image_dimension[0];
            $image_height    = $image_dimension[1];
            $image_dimension_for_db = $image_width . ' x ' . $image_height . ' pixels';
            $image_size_for_db      = $image->getSize();

            // Prepare the image file name components
            $image_extension     = $image->getClientOriginalExtension();
            $image_name_with_ext = $image->getClientOriginalName();
            $image_name          = pathinfo($image_name_with_ext, PATHINFO_FILENAME);
            $image_name          = strtolower(Str::slug($image_name));

            // Create unique names for various image sizes
            $timestamp = time();
            $image_db         = $image_name . $timestamp . '.' . $image_extension;
            $image_grid       = 'grid-' . $image_db;
            $image_large      = 'large-' . $image_db;
            $image_thumb      = 'thumb-' . $image_db;
            $image_semi_large = 'semi-large-' . $image_db;
            ...
            // Define the folder path using public_path() to get an absolute path
            $folder_path = 'assets/uploads/media-uploader/';;


            // Create resized images using the absolute image path
            $resize_grid_image = Image::make($imagePath)->resize(350, null, function ($constraint) {
                $constraint->aspectRatio();
            });
            $resize_large_image = Image::make($imagePath)->resize(740, null, function ($constraint) {
                $constraint->aspectRatio();
            });
            $resize_semi_large_image = Image::make($imagePath)->resize(540, 350, function ($constraint) {
                $constraint->aspectRatio();
            });
            $resize_tiny_image = Image::make($imagePath)->resize(15, 15)->blur(50);
            $resize_thumb_image = Image::make($imagePath)->resize(150, 150);
            $resize_full_image = Image::make($imagePath)->resize($image_width, $image_height, function ($constraint) {
                $constraint->aspectRatio();
            });

            // Save the full image in the main folder
            $resize_full_image->save($folder_path . '/' . $image_db);

            // Save image details in the database (adjust this as needed for your app)
            MediaUpload::create([
                'title'      => $image_name_with_ext,
                'size'       => formatBytes($image_size_for_db),
                'path'       => $image_db,
            ...
            // If the image width is greater than 150, create additional resized versions
            if ($image_width > 150) {
                $thumb_image_folder       = $folder_path . '/thumb';
                $grid_image_folder        = $folder_path . '/grid';
                $large_image_folder       = $folder_path . '/large';
                ...
                foreach ([$thumb_image_folder, $grid_image_folder, $large_image_folder, $semi_large_image_folder, $tiny_image_folder] as $subfolder) {
                    if (!is_dir($subfolder)) {
                        mkdir($subfolder, 0777, true);
                    }
                }

                // Save the resized images in their respective folders
                $resize_thumb_image->save($thumb_image_folder . '/' . $image_thumb);
                $resize_grid_image->save($grid_image_folder . '/' . $image_grid);
                $resize_large_image->save($large_image_folder . '/' . $image_large);
                $resize_semi_large_image->save($semi_large_image_folder . '/' . $image_semi_large);
                $resize_tiny_image->save($tiny_image_folder . '/' . $image_tiny);
            }
        }
    }
