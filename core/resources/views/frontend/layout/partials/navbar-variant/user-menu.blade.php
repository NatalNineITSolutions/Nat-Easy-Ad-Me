    ...
    @if(Auth::check())
        <li class="single">
            <div class="btn-wrapper">
                @if(($global_reached_limit ?? false) || ($global_remaining_listings ?? 1) === 0)
                    <a href="{{ route('user.membership.all') }}" class="cmn-btn1">
                        <i class="las la-crown"></i>
                        <span class="text">{{ __('Upgrade to Post More') }}</span>
                    </a>
                @else
