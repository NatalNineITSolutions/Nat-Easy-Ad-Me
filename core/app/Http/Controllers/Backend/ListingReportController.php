...
class ListingReportController extends Controller
{
    ...
    public function listingExpiryDateView()
    {
        return view('backend.pages.listings.expiry-settings', [
            'listing_expiry_days' => get_static_option('listing_expiry_days', 30),
        ]);
    }

    public function listingExpiryDate(Request $request)
    {
        $request->validate([
            'expiry_days' => 'required|integer|min:1|max:365',
        ]);

        update_static_option('listing_expiry_days', $request->expiry_days);

        return back()->with(['msg' => 'Expiry duration updated successfully!', 'type' => 'success']);
    }
}
