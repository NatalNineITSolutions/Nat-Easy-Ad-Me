<table class="DataTable_activation">
    ...
    @foreach($all_cities as $city)
        <tr>
            ...
            {{-- ✅ District --}}
            <td>{{ optional($city->district)->district }}</td>
            ...
            <td>
                @can('city-edit')
                    <a class="cmnBtn btn_5 btn_bg_warning radius-5 edit_city_modal"
                       ...
                       data-district_id="{{ $city->district_id }}"> {{-- ✅ NEW --}}
                        {{ __('Edit City') }}
                    </a>
                @endcan
                ...
                @can('city-delete')
                    <x-popup.delete-popup
                        ...
                        :url="route('admin.city.delete', $city->id)"/>
                @endcan

                @can('city-status-change')
                    <x-status.table.status-change
                        :title="__('Change Status')"
                        :url="route('admin.city.status', $city->id)"/>
                @endcan
            </td>
        </tr>
    @endforeach
